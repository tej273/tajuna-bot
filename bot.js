...
async function safeSend(fn) {
  try { return await fn(); } catch (e) { console.error('Send error (ignored):', e.message); }
}
...
bot.start(async (ctx) => {
  ...
  } catch (e) { console.error('Start error (ignored):', e.message); }
});
...
bot.action('start_main', async (ctx) => {
  ...
  } catch (e) { console.error('Menu error (ignored):', e.message); }
});
...
bot.action('plan_390', async (ctx) => {
  try {
    ...
    if (fs.existsSync(QR_390_PATH)) {
      await ctx.replyWithPhoto({ source: QR_390_PATH }, {
        caption: details, parse_mode: 'Markdown', ...backKeyboard(),
      });
    } else {
  ...
  } catch (e) { console.error('Plan 390 error (ignored):', e.message); }
});
...
bot.action('plan_2000', async (ctx) => {
  ...
  } catch (e) { console.error('Plan 2000 error (ignored):', e.message); }
});
...
bot.on('photo', async (ctx) => {
  ...
  } catch (e) { console.error('Photo error (ignored):', e.message); }
});
...
bot.action(/^approve_(\d+)_(.+)$/, async (ctx) => {
  try {
    ...
    db.users[userId] = {
      userId, plan, approvedAt: Date.now(),
      username: approval.username, firstName: approval.firstName,
    };
    saveDB(db);
    ...
    try {
      ...
      console.error('Link generation error (ignored):', e.message);
    }
    ...
    if (inviteLink) {
      ...
      await safeSend(() =>
        bot.telegram.sendMessage(
          ...
          `⚠️ Contact admin directly to get your group link.`,
          { parse_mode: 'Markdown', ...mainMenuKeyboard() }
        )
      );
    }
  ...
  } catch (e) { console.error('Approve error (ignored):', e.message); }
});
...
bot.action(/^reject_(\d+)_(.+)$/, async (ctx) => {
  ...
  } catch (e) { console.error('Reject error (ignored):', e.message); }
});
...
bot.on('message', async (ctx) => {
  ...
  } catch (e) { console.error('Message error (ignored):', e.message); }
});
